    ...
    async function handleFreelancerRegister(values){
        ...
        try {
            await api.post('users', data).then(response => {
                ...
                const userSession = { user_id: response.data.user._id, user_name: response.data.user.name, user_is_freelancer: response.data.user.is_freelancer, user_has_team: false }
